<div class="py-5  bg-slate-800  dark:bg-gray-100 ">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 ">
        ...
        <div class="col-xs-12">
                        ...
                        <div class="py-5 bg-white dark:bg-gray-100 rounded-md">
                            <div x-data="{ open: false }" class="shadow-lg">
                                <div @click="open = !open"
                                    ...
                                    <p class="text-lg font-medium text-white">D.C.R.G</p>
                                        ...
                                        <div class="grid grid-cols-3 gap-3 mb-10">
                                            <div class="mt-2">
                                                <label for="dependencia_riogrande_id"
                                                    class="block text-sm font-medium text-gray-700">Dependencias Rio
                                                    Grande
                                                </label>
                                                <select class="w-full form-control"
                                                    ...
                                                    @foreach ($Dependencia_RioGrande as $rg)
                            ...
                            <div x-data="{ open: false }" class="shadow-lg">
                                <div @click="open = !open"
                                    class="flex items-center justify-between bg-slate-800 border p-4 rounded-md transition">
                                    <p class="text-lg font-medium text-white">INVENTARIO D.C.R.G</p>
                                    <span :class="open ? 'fa-chevron-down' : 'fa-chevron-up'" class="fas"></span>
                                </div>
                                <div x-show.transition.in.duration.800ms="open" class="border p-4">
                                    ...
                                    <div class="grid grid-cols-4 gap-2 mb-10">
                                        <div>
                                            <h1 class="text-red-800 text-lg font-bold mb-2">Total de equipos "Ht":
                                                {{ $HtCount }}</h1>
                                            <p class="text-purple-800 font-bold mb-2">Ht Otros:
                                                {{ $marcaotros }} </p>
                                            <p class="text-purple-800 font-bold mb-2">Ht Motorola:
                                                {{ $marcaMotorola }} </p>
                                            <p class="text-purple-800 font-bold mb-2">Ht Kenwood: {{ $marcaKenwood }}
                                            </p>
                                            <p class="text-purple-800 font-bold mb-2">Ht Yaesu: {{ $marcaYaesu }}
                                            </p>
                                            <p class="text-purple-800 font-bold mb-2">Ht Hytera: {{ $marcaHytera }}
                                            </p>
                                        </div>
                                        <div>
                                            <h1 class="text-red-800 font-extrabold mb-2">Total de "Equipos Base":
                                                {{ $BaseCount }}</h1>
                                            <p class="text-purple-800 font-bold mb-2">Equipo Base Otros:
                                                {{ $baseotros }} </p>
                                            <p class="text-purple-800 font-bold mb-2">Equipo Base Motorola:
                                                {{ $baseMotorola }}</p>
                                            <p class="text-purple-800 font-bold mb-2">Equipo Base Kenwood:
                                                {{ $baseKenwood }}</p>
                                            <p class="text-purple-800 font-bold mb-2">Equipo Base Yaesu:
                                                {{ $baseYaesu }}</p>
                                            <p class="text-purple-800 font-bold mb-2">Equipo Base Hytera:
                                                {{ $baseHytera }}</p>
                                        </div>
                                        <div>
                                            <h1 class="text-red-800 font-extrabold mb-2">Total de "Antenas":
                                                {{ $AntenaCount }}</h1>
                                            <p class="text-purple-800 font-bold mb-2">Antena "Otros":
                                                {{ $Otros }}</p>
                                            <p class="text-purple-800 font-bold mb-2">Antena Dipolo 2 elementos:
                                                {{ $dipolo2 }}</p>
                                            <p class="text-purple-800 font-bold mb-2">Antena Dipolo 4 elementos:
                                                {{ $dipolo4 }}</p>
                                            <p class="text-purple-800 font-bold mb-2">Antena Dipolo 8 elementos:
                                                {{ $dipolo8 }}</p>
                                            <p class="text-purple-800 font-bold mb-2">Antena Dipolo Yagi:
                                                {{ $yagi }}</p>
                                            <p class="text-purple-800 font-bold mb-2">Antena Dipolo Latigo:
                                                {{ $latigo }}</p>
                                        </div>
                            ...
                            <div x-data="{ open: false }" class="shadow-lg">
                                <div @click="open = !open"
                                    class="flex items-center justify-between bg-slate-800 border p-4 rounded-md transition">
                                    <p class="text-lg font-medium text-white">TABLA DE INVENTARIO</p>
                                    <span :class="open ? 'fa-chevron-down' : 'fa-chevron-up'" class="fas"></span>
                                </div>
                                <div x-show.transition.in.duration.800ms="open" class="border p-4">
            ...
            @livewire('dcrgcomu.riograndecomu.index-comunicaciones-riogrande')
        </div>
