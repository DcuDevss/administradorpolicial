<div>
    ...
    <div class="mb-4 mt-8">
        <button type="button"
                class="btn-pdf"
                onclick="descargarPdfAdministracion()"
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px; background:#ffffff; color:#000000;">
            ...
            <tbody style="background:#ffffff;color:#000000;">
                @forelse($registros as $r)
                    <tr>
                        <td style="border:1px solid #ddd;padding:6px;color:#000000;">{{ $r->tipo ?? '-' }}</td>
                        <td style="border:1px solid #ddd;padding:6px;color:#000000;">{{ $r->oficina ?? '-' }}</td>
                        <td style="border:1px solid #ddd;padding:6px;color:#000000;">{{ $r->marca ?? '-' }}</td>
                        <td style="border:1px solid #ddd;padding:6px;color:#000000;">{{ $r->detalles_inventario ?? '-' }}</td>
    ...
    <div wire:ignore>
        ...
        <script>
            window.descargarPdfAdministracion = function () {
                const el = document.getElementById('pdf-content');
                if (!el) return;

                html2pdf().set({
                    margin: 0,
                    filename: `inventario_administracion_${Date.now()}.pdf`,
