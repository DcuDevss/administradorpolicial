<div
 ...
 class="flex flex-col transition-all h-full overflow-hidden">
    ...
    <header class="px-3 z-10 bg-gray-300 sticky top-0 w-full py-2">

        <div class="border-b justify-between flex items-center pb-2">

            <div class="flex items-center gap-2">
                 <h5 class="font-extrabold text-2xl text-red-600">Dialogo servicios policiales</h5>
            </div>
        ...
        <div class="flex gap-3 items-center overflow-x-scroll p-2 bg-gray-300">
            ...
            <button @click="type='all'" :class="{'bg-blue-300 border-0 text-black':type=='all'}" class="inline-flex justify-center items-center rounded-full gap-x-1 text-xs font-medium px-3 lg:px-5 py-1  lg:py-2.5 border ">
                    Todo
            </button>
            <button @click="type='deleted'" :class="{'bg-blue-300 border-0 text-black':type=='deleted'}" class="inline-flex justify-center items-center rounded-full gap-x-1 text-xs font-medium px-3 lg:px-5 py-1  lg:py-2.5 border ">
                Borrar
            </button>
    ...
    <main class=" overflow-y-scroll overflow-hidden grow  h-full relative " style="contain:content">
        ...
        <ul class="p-2 grid w-full spacey-y-2">
            ...
            <li
             ...
             class="py-3 text-purple-600 rounded-2xl  transition-colors duration-150 flex gap-4 relative w-full cursor-pointer px-2 {{$conversation->id==$selectedConversation?->id ? 'bg-white':''}}">
                ...
                <aside class="grid grid-cols-12 w-full">
                    ...
                    <a href="{{route('chat',$conversation->id)}}" class="col-span-11 border-b pb-2 border-gray-200 relative overflow-hidden truncate leading-5 w-full flex-nowrap p-1">
                        ...
                        <div class="flex justify-between w-full items-center rounded-md">

                            <h6 class="truncate font-medium tracking-wider text-gray-900">
                                {{$conversation->getReceiver()->name}}
                            </h6>

                            <small class="text-gray-900">{{$conversation->messages?->last()?->created_at?->shortAbsoluteDiffForHumans()}} </small>
