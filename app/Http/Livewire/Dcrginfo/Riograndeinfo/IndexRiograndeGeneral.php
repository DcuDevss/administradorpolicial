...
class IndexRiograndeGeneral extends Component
{
    use WithPagination;
    public $buscar='';
    public $perPage1 = '4';
    ...
    {
        $rg = Riograndegenerale::where(function ($query) {
        ...
        ->orWhereHas('dependencia_riogrande', function ($query) {
            $query->where('nombre', 'like', "%{$this->search}%");
        })
        ->orWhereHas('tipodeoficina', function ($query) {
            $query->where('nombre', 'like', "%{$this->search}%");
        })
        ->orWhereHas('riogrande', function ($query) {
            $query->where('nombre', 'like', "%{$this->search}%");
        })
        ...
        return view('livewire.dcrginfo.riograndeinfo.index-riogrande-general',compact('rg'));
    }
