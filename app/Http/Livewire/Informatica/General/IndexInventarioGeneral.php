...
class IndexInventarioGeneral extends Component
{
    use WithPagination;
    public $buscar='';
    public $perPage1 = '4';
    ...
    public function render()
    {
       ...
       // $investigacion = Investigacionesgenerale::all();
        ...
        $general = Generalinformatica::where(function ($query) {
            $query->where('marca', 'like', "%{$this->search}%")
                ->orWhere('modelo', 'like', "%{$this->search}%")
                ->orWhere('procesador', 'like', "%{$this->search}%")
               // ->orWhere('monitor', 'like', "%{$this->search}%")
        ...
        ->orWhereHas('serviciosespeciale', function ($query) {
            $query->where('nombre', 'like', "%{$this->search}%");
        })
        ->orWhereHas('destacamento', function ($query) {
            $query->where('nombre', 'like', "%{$this->search}%");
        })
        /*->orWhereHas('cientifica', function ($query) {
            $query->where('nombre', 'like', "%{$this->search}%");
        })*/
        ->orWhereHas('custodiagubernamentale', function ($query) {
            $query->where('nombre', 'like', "%{$this->search}%");
        })
        /*->orWhereHas('administracion', function ($query) {
            $query->where('nombre', 'like', "%{$this->search}%");
        })*/
        ->orWhereHas('tipodispositivo', function ($query) {
            $query->where('nombre', 'like', "%{$this->search}%");
        })
        ->orWhereHas('tipodeoficina', function ($query) {
            $query->where('nombre', 'like', "%{$this->search}%");
        })
        ...
        //$general=Generalinformatica::all();,'investigacion'
        return view('livewire.informatica.general.index-inventario-general',compact('general'));
    }
